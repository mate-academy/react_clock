...
1) Status has CURRENT_TIME, HAS_CLOCK == true, CLOCK_NAME===0
2) In method componentDidMount create:
  a) setInterval that will update CURRENT_TIME every second on the page and in log
  b) setInterval that will update CLOCK_NAME every 3 second on the page
  c) eventListener('contextmen') that will update hasClock  false
  d) eventListener('click') that will update hasClock true 
3) In method componentDidUpdate check if CLOCK_NAME was changed then show prev and current CLOCK_NAME in log
...
- Replace `<your_account>` with your Github username in the [DEMO LINK](https://vanyachyzh.github.io/react_clock/) and add it to the PR description.
